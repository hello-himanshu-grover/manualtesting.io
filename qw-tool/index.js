...
// Store Tset Coverage
const storeTestCoverage = async (branch, coverage, report) => {
	try {
		// await axios.get(`http://35.168.193.209:4000/updateCoverage/${branch}/${coverage}`);
		return;
	} catch (err) {
		console.log(err);
	}
...
const extractUnTestedFunctions = async ({ reportPath, resultPath }) => {
	...
	await storeTestCoverage(branchName, Math.round(100 - ((unTestedLineCount / fileLineCount) * 100)), report);
